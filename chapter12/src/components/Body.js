            ...
            (restaurantList.length === 0 && errMsg === "") ?
                ...
                <>
                    <div className="flex gap-2 justify-end m-8">
                        <input className="border w-72 p-2 rounded-md" type="text" name="search" id="search" value={searchText} onChange={(e) => setSearchText(e.target.value)} />
                        <button className="border p-2 bg-gray-200 rounded-md" onClick={() => {
                            const data = filterData(searchText, restaurantList);
                            setFilteredRestList(data);
                        }}>Search</button>
                    </div>
                    <div className='flex flex-wrap justify-center gap-24 min-h-[68vh]'>
                        {
